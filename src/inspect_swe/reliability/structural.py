"""Structural reliability phase execution scaffolding."""
...
from .perturbations import StructuralPerturbationSpec
from .spec import ReliabilitySpec
from .telemetry import TelemetryCoverageReport, assess_sidecar_coverage


class StructuralExecutionError(RuntimeError):
    """Raised when structural phase execution violates reliability constraints."""


class StructuralPhaseConfig(BaseModel):
    """Options for structural phase execution."""
    ...
    perturbation: StructuralPerturbationSpec = Field(
...
class StructuralRepeatResult(BaseModel):
    """One structural repeat execution summary."""
...
class StructuralPhaseResult(BaseModel):
    """Complete structural phase execution summary."""
...
def run_structural_phase(
    *,
    ...
    """Run K independent structural repeats for all agents in the spec."""
    config = config or StructuralPhaseConfig()
    if "structural" not in spec.phases:
        raise StructuralExecutionError(
            "ReliabilitySpec does not include structural phase; refusing structural run."
        )
...
def _resolve_sidecar_path(
    spec: ReliabilitySpec, config: StructuralPhaseConfig, campaign_id: str
) -> str:
