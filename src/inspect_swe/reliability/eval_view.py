...
class BaselineSampleView(BaseModel):
    """Normalized baseline view derived from one eval sample epoch."""
    ...
    def identity_key(self) -> str:
        return (
            ...
            f"{self.sample_uuid}:e{self.epoch}:sr{self.sample_retry_id}:aa{self.agent_attempt_id}"
        )
...
def extract_baseline_sample_views(
    ...
    *,
    ...
    """Extract normalized baseline epoch views from an eval log."""
    ...
    for sample in samples:
        ...
        epoch = _read_int(getattr(sample, "epoch", 1), 1)
        ...
        if strict_identity_tags:
            if phase != expected_phase or agent != expected_agent:
                warnings += 1
...
def _extract_confidence(
    *,
    ...
    del sample, metadata
    for key in ("reliability_confidence",):
