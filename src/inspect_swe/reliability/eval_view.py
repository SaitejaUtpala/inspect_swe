...
class BaselineSampleView(BaseModel):
    """Normalized telemetry view derived from one eval sample."""
...
def extract_baseline_sample_views(
    ...
    *,
    ...
    """Extract normalized telemetry views from an eval log."""
    ...
    for sample in samples:
        ...
        if strict_identity_tags and (
            phase != expected_phase
            or repeat_id != expected_repeat_id
...
def _normalize_scores(scores: dict[str, Any]) -> dict[str, Any]:
    return {name: getattr(score, "value", score) for name, score in scores.items()}
